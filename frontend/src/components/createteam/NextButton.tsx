...
export default function CreateTeamButton({ name, slug, logoUrl }: Props) {
  ...
  const handleCreateTeam = async () => {
    ...
    try {
      ...
      console.log("📤 [CreateTeamButton] Calling POST /teams with:", {
        name: name.trim(),
        slug: slug.trim(),
      ...
      console.log("✅ [CreateTeamButton] /teams response:", data);

      const created = data?.data || data;
      const teamId = created?.id;

      if (!teamId) {
        console.log("❌ [CreateTeamButton] No team ID found in response", data);
        Alert.alert("Error", "Team created but ID missing in response");
        return;
      }

      await AsyncStorage.setItem("@last_created_team_id", teamId);
      console.log("💾 [CreateTeamButton] Saved team ID:", teamId);

      Alert.alert("Success", "Team created successfully");

      console.log("➡️ [CreateTeamButton] Navigating to MyTeam");
      navigation.navigate("MyTeam");
    } catch (err: any) {
      console.log("🚨 [CreateTeamButton] Create team failed:", err);
