...
const CARD_W = width - H_PADDING * 4; // full width inside section padding
const CARD_H = 200;
const CARD_R = 20;
    ...
    >
      ...
      <View style={styles.rowBetween}>
        <View style={styles.sideBox}>
          ...
          {!!data.left.score && (
            <Text style={styles.sideScore}>{data.left.score}</Text>
          )}
        ...
        <View style={[styles.sideBox, { alignItems: "flex-end" }]}>
          ...
          {!!data.right.score && (
            <Text style={styles.sideScore}>{data.right.score}</Text>
          )}
          {!!data.right.ov && (
            <Text style={styles.sideOv}>{data.right.ov}</Text>
          )}
        </View>
      </View>
      ...
      <View style={[styles.infoCard]}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <View style={{ alignItems: "flex-end" }}>
            ...
            <Pressable
              onPress={onPressToss}
              style={({ pressed }) => [styles.tossBtn, pressed && { opacity: 0.9 }]}
            >
...
function UpcomingFixtures({
  ...
  const items = useMemo(
    () => [
      { type: "hero" as const, data: hero },
      ...infoCards.map((it) => ({ type: "info" as const, data: it })),
    ],
    [hero, infoCards]
  );
...
const styles = StyleSheet.create({
  ...
  // Info card (same height & radius)
  infoBorder: { borderRadius: CARD_R, padding: 1.2 },
  infoCard: {
    height: CARD_H,          // 🔸 same height
    borderRadius: CARD_R,    // 🔸 same radius
